...
class SimpleDataset:
  ...
  def train_data_subset(self, train_obs):
    """Optionally restricts the training data to a psuedorandom subset.
    
    If the args['dataset']['dataset_size'] integer is smaller than the size
    of the dataset, then picks a pseudorandom subset of the training data
    to use as training. Requires same seed for comparable results.

    Args:
      train_obs: the training observations to pick a subset of
    Returns:
      The subset of the training dataset to be used.
    """
