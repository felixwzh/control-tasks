...
EWT_TRAIN_EMPIRICAL_POS_DISTRIBUTION=[0.01627494391742299, 0.00397343225371318, 0.000825965622235581, 0.0013489142706332566, 0.019525045330361812, 0.05960637117624347, 0.01939308632562595, 7.331055818659004e-05, 0.000855289845510217, 0.02626472931298232, 0.03278936899158884, 0.01605989961340899, 0.004755411541036807, 0.0005718223538554023, 0.0002345937861970881, 0.022384157099638823, 0.05295954723399264, 0.003245214042393052, 0.0014368869404571647, 0.05667883621932564, 0.0036899647620583648, 0.02640646305880973, 0.00463322727739249, 0.004232462892639131, 0.08219579783880475, 0.0037144016147872285, 0.014974903352247458, 0.0024339105317947893, 0.0033722856765831415, 0.001651931244471162, 0.004247125004276449, 0.003836585878431545, 0.0017545660259323881, 0.0012609416008093485, 0.03940198134001926, 0.131465380310739, 0.01609899857777517, 0.0018718629190309323, 0.0033429614533085055, 0.05042300192073663, 0.004921582139593077, 4.887370545772669e-06, 0.04127873162959596, 0.0014271121993656194, 0.0007868666578693997, 0.04631272329174181, 0.10125654296731815, 0.06083310118323241, 0.0024583473845236526, 0.0004545254607568582]
...
class RandomParseDepthTask(Task):
  ...
  def __init__(self, args):
    self.vocab = {}
    self.args = args
    if args['dataset']['corpus']['name']=='en_ewt-ud':
      self.dist = PTB_DEV_EMPIRICAL_DEPTH_DISTRIBUTION
    elif args['dataset']['corpus']['name']=='ptb':
      self.dist = PTB_DEV_EMPIRICAL_DEPTH_DISTRIBUTION
    args['ignore_punct'] = True 
...
class CorruptedPartOfSpeechLabelTask(PartOfSpeechLabelTask):
  ...
  def __init__(self, args):
    ...
    self.rand_type_vocab = {}
    if args['dataset']['corpus']['name']=='en_ewt-ud':
      self.dist = EWT_TRAIN_EMPIRICAL_POS_DISTRIBUTION
    elif args['dataset']['corpus']['name']=='ptb':
  ...
  def _register_type(self, string):
    if string not in self.rand_type_vocab:
      if self.args['dataset']['corpus']['name']=='en_ewt-ud':
        ints = list(range(50))
      elif self.args['dataset']['corpus']['name']=='ptb':
        ints = list(range(45))
      self.rand_type_vocab[string] = int(np.random.choice(ints, p=self.dist))
    return self.rand_type_vocab[string]
  ...
  def labels(self, observation):
    ...
    if self.args['dataset']['corpus']['name']=='en_ewt-ud':
      self.args['probe']['label_space_size'] = 50
    elif self.args['dataset']['corpus']['name']=='ptb':
